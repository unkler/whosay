            ...
            <div class="lg:flex items-center">
                <select name="category" class="mb-2 lg:mb-0 lg:mr-2">
                    ...
                    <optgroup label="{{ $category->name }}">
                        ...
                        <option value={{ $secondary->id }} @if(\Request::get('category') === (String)$secondary->id) selected @endif>{{ $secondary->name }}</option>
                        @endforeach
                    @endforeach
                </select>
                <div class="flex space-x-2 items-center">
                    <div><input name="keyword" class="border border-gray-500 py-2 pl-2" placeholder="キーワードを入力して下さい"></div>
                    <div><button class="text-white bg-indigo-500 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-600">検索</button></div>
                </div>
            </div>
            <div class="flex">
                <div>
