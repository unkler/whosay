              ...
              <div class="p-6 bg-white border-b border-gray-200">
                <x-flash-message />
                    ...
                    <div class="-m-2">
                      ...
                      <div class="p-2 w-1/2 mx-auto">
                        ...
                        <div class="p-2 w-1/2 mx-auto">
                          <div class="relative flex justify-around">
                              <div>
                                  <input type="radio" name="type" value="{{ \Constant::PRODUCT_LIST['add'] }}" class="mr-2" checked {{ old('type') === \Constant::PRODUCT_LIST['add'] ? 'checked' : '' }}>追加
                              </div>
                              <div>
                                  <input type="radio" name="type" value="{{ \Constant::PRODUCT_LIST['reduce'] }}" class="mr-2" {{ old('type') === \Constant::PRODUCT_LIST['reduce'] ? 'checked' : '' }} >削減
                              </div>
                          </div>
                        </div>
                      </div>
                      <div class="p-2 w-1/2 mx-auto">
                        <div class="relative">
                          ...
                          <input type="number" id="price" name="quantity" value="{{ old('quantity', 0) }}" required class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                        ...
                        <div class="relative">
                          ...
                          <select name="shop_id" id="shop_id" class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                            @foreach ($shops as $shop)
                              <option value="{{ $shop->id }}" @if($shop->id === (int)old('shop_id', $product->shop_id)) selected @endif>{{ $shop->name }}</option>
                            @endforeach
                          </select>
                        </div>
                      </div>
                              ...
                              @foreach ($categories as $category)
                                <optgroup label="{{ $category->name }}">
                                  @foreach ($category->secondary as $secondary)
                                    <option value={{ $secondary->id }} @if($secondary->id === (int)old('category', $product->secondary_category_id)) selected @endif>{{ $secondary->name }}</option>
                                  @endforeach
                              @endforeach
                            </select>
                          </div>
                      ...
                      <div class="p-2 w-1/2 mx-auto">
                        <div class="relative flex justify-around">
                            <div>
                                <input type="radio" name="is_selling" value="1" class="mr-2" checked {{ old('is_selling', (String)$product->is_selling) === '1' ? 'checked' : '' }}>販売中
                            </div>
                            <div>
                                <input type="radio" name="is_selling" value="0" class="mr-2" {{ old('is_selling', (String)$product->is_selling) === '0' ? 'checked' : '' }} >停止中
                            </div>
                        </div>
                      </div>
                 ...
                 <form id="delete_{{ $product->id }}" method="POST" action="{{ route('owner.products.destroy', ['product' => $product->id]) }}">
                  ...
                  @csrf
                  <div class="p-2 w-full flex justify-around mt-4">
                    <a href="#" data-id="{{ $product->id }}" onclick="deletePost(this)" class="text-white bg-gray-400 border-0 py-2 px-4 focus:outline-none hover:bg-gray-500 rounded">削除</a>
                  </div>
                </form>
              </div>
          </div>
